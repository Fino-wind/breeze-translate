...
async function getStoredState() {
  await stateWriteQueue.catch(function ignoreQueueError() {
    return null;
  });
...
async function saveStoredState(partialState) {
  const writeTask = stateWriteQueue.then(async function writeState() {
    ...
    await storage.set({
      [core.STORAGE_KEY]: nextState,
    });

    return nextState;
  });

  stateWriteQueue = writeTask.catch(function swallowQueueError() {
    return null;
  });
  return writeTask;
}
...
function pruneSelectionCache() {
  if (selectionTranslationCache.size <= 50) {
    return;
  }

  const firstKey = selectionTranslationCache.keys().next();
  if (!firstKey.done) {
    selectionTranslationCache.delete(firstKey.value);
  }
}
...
function createContextMenus() {
  chrome.contextMenus.removeAll(function clearMenus() {
    chrome.contextMenus.create({
      id: CONTEXT_MENU_KNOWN,
      title: '加入认识词库',
      contexts: ['selection'],
    }, function handleCreateKnown() {
      if (chrome.runtime.lastError) {
        appendLog('warn', 'context-menu', chrome.runtime.lastError.message, { id: CONTEXT_MENU_KNOWN });
      }
    });
    chrome.contextMenus.create({
      id: CONTEXT_MENU_LEARNING,
      title: '加入不认识词库',
      contexts: ['selection'],
    }, function handleCreateLearning() {
      if (chrome.runtime.lastError) {
        appendLog('warn', 'context-menu', chrome.runtime.lastError.message, { id: CONTEXT_MENU_LEARNING });
      }
    });
  });
}
...
async function appendLog(level, event, message, detail) {
  const writeTask = logWriteQueue.then(async function writeLog() {
    ...
    await storage.set({
      [core.LOG_STORAGE_KEY]: core.appendDebugLog(logs, entry, core.MAX_LOG_ENTRIES),
    });

    return entry;
  });

  logWriteQueue = writeTask.catch(function swallowQueueError() {
    return null;
  });
  return writeTask;
}
...
async function clearLogs() {
  const clearTask = logWriteQueue.then(function writeClear() {
    return storage.set({
      [core.LOG_STORAGE_KEY]: [],
    });
  });

  logWriteQueue = clearTask.catch(function swallowQueueError() {
    return null;
  });
  await clearTask;
}
...
async function requestAnnotation(text, overrides, logContext) {
  ...
  const mode = (state.ui && state.ui.mode) || 'full';

  if (!state.settings.baseUrl || !state.settings.model) {
    const result = {
      ok: false,
      error: '请先在 Options 页面填写 Base URL 和 Model Name。',
    };
  ...
  try {
    ...
    const result = {
      ok: false,
      error: '网络请求失败：' + (error && error.message ? error.message : '未知错误'),
    };
    await appendLog('error', logContext || 'annotation-error', result.error, {
      baseUrl: state.settings.baseUrl,
      model: state.settings.model,
  ...
  if (!response.ok) {
    const result = {
      ok: false,
      error: '模型请求失败：' + readErrorMessage(payload, 'HTTP ' + response.status),
    };
    await appendLog('error', logContext || 'annotation-error', result.error, {
      baseUrl: state.settings.baseUrl,
      model: state.settings.model,
  ...
  let annotatedText = '';
  try {
    annotatedText = core.extractAssistantText(payload);
  } catch (error) {
    const result = {
      ok: false,
      error: '模型返回内容格式无效。',
...
async function requestBatchAnnotation(texts, overrides, logContext) {
  ...
  if (!parsed) {
    const result = {
      ok: false,
      error: '模型返回的批量结果无法按分隔符拆分。',
    };
    await appendLog('error', logContext || 'annotation-batch-error', result.error, {
      count: blocks.length,
      preview: core.createPreviewText(singleResult.text, 160),
    });
    return result;
  }
...
async function testConnection(settings) {
  const state = core.mergeStoredState({
    settings: settings,
    vocabulary: TEST_CONNECTION_VOCABULARY,
  });

  if (!state.settings.baseUrl || !state.settings.model) {
    const missingConfig = {
      ok: false,
      error: '请先填写 Base URL 和 Model Name。',
    };
  ...
  let outputText = '';
  try {
    outputText = core.extractAssistantText(payload);
  } catch (error) {
    const parseError = {
      ok: false,
      error: '模型返回内容格式无效。',
...
chrome.runtime.onInstalled.addListener(function handleInstalled() {
  initializeStorage().catch(function ignoreError() {
    return null;
  });
  createContextMenus();
});
...
chrome.contextMenus.onClicked.addListener(function handleContextMenu(info) {
  ...
  if (info.menuItemId === CONTEXT_MENU_KNOWN) {
    addWordToVocabulary(selectedText, 'knownWords').catch(function ignoreError() {
      return null;
    });
    return;
  }
  ...
  if (info.menuItemId === CONTEXT_MENU_LEARNING) {
    addWordToVocabulary(selectedText, 'learningWords').catch(function ignoreError() {
      return null;
    });
  }
});
...
chrome.runtime.onMessage.addListener(function handleMessage(message, _sender, sendResponse) {
  ...
  if (message.type === core.MESSAGE_TYPES.GET_STATE) {
    getStoredState()
      ...
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '读取数据失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.GET_LOGS) {
    getStoredLogs()
      .then(function resolveLogs(logs) {
        sendResponse({ ok: true, logs: logs });
      })
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '读取日志失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.SAVE_SETTINGS) {
    saveStoredState({ settings: message.settings })
      ...
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '保存配置失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.TEST_CONNECTION) {
    testConnection(message.settings)
      .then(function resolveTest(result) {
        sendResponse(result);
      })
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '连接测试失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.SAVE_VOCABULARY) {
    saveStoredState({ vocabulary: message.vocabulary })
      ...
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '保存词库失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.ADD_WORD_TO_LIST) {
    addWordToVocabulary(message.word, message.listKey)
      ...
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '写入词库失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.TRANSLATE_SELECTION) {
    translateSelection(message.text)
      .then(function resolveTranslation(result) {
        sendResponse(result);
      })
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '划词翻译失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.ANNOTATE_TEXT) {
    requestAnnotation(message.text)
      .then(function resolveAnnotation(result) {
        sendResponse(result);
      })
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '注词失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.ANNOTATE_BATCH) {
    requestBatchAnnotation(message.texts, message.overrides, 'annotation-batch-error')
      .then(function resolveBatch(result) {
        sendResponse(result);
      })
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '批量注词失败。' });
      });
  ...
  if (message.type === core.MESSAGE_TYPES.CLEAR_LOGS) {
    clearLogs()
      ...
      .catch(function handleError(error) {
        sendResponse({ ok: false, error: error && error.message ? error.message : '清空日志失败。' });
      });
  ...
  if (message.type === 'NF_AUDIO_TOGGLE') {
    toggleAudioCapture(message.serverUrl)
      .then(function resolveToggle(result) { sendResponse(result); })
      .catch(function handleError(error) { sendResponse({ ok: false, error: error.message }); });
    return true;
  }
  ...
  if (message.type === 'NF_OFFSCREEN_READY') {
    if (pendingAudioStart) {
      chrome.runtime.sendMessage(pendingAudioStart);
